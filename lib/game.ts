export type Phase='lobby'|'supply'|'negotiation'|'secret'|'result'|'guess'|'finished';
...
export type Player={id:string;token:string;name:string;role:string;inventory:Energy[];converted:boolean;transferred:boolean;pendingTransfer?:{toId:string;energyKey:EnergyKey};inbox:string[];stats:{converted:number;sent:number;received:number;car:number;paths:string[]};guess?:string;guesses?:{toId:string;role:string}[]};
export type Room={code:string;group:number;set:string;phase:Phase;round:number;deadline:number|null;players:Player[];paths:string[];pathProgress:Record<string,number>;supplyRecipients:Record<string,number>;log:string[]};
...
export function newSession():Session{const code=String(Math.floor(1000+Math.random()*9000));return{code,teacherToken:uid(),createdAt:Date.now(),negotiationSeconds:60,rooms:Array.from({length:4},(_,i)=>({code:`${code}-${i+1}`,group:i+1,set:'균형형',phase:'lobby',round:0,deadline:null,players:[],paths:[],pathProgress:{},supplyRecipients:{},log:[]}))}}
export function normalizeSession(s:Session){s.negotiationSeconds=Math.min(300,Math.max(60,s.negotiationSeconds||60));s.rooms.forEach(r=>{r.pathProgress||={};r.paths||=[];r.supplyRecipients||={};r.players.forEach(p=>{p.guesses||=p.guess?[{toId:p.guess.split('|')[0],role:p.guess.split('|')[1]}]:[]})});return s}
...
export function start(room:Room){if(room.players.length<8)throw Error('최소 8명이 참여해야 합니다.');const roles=rolesFor(room.players.length);room.players.forEach((p,i)=>{p.role=roles[i];if(p.role==='화석연료')p.inventory.push({id:uid(),type:'화학E',path:'태양→식물→화석연료',createdBy:p.id,initial:true})});room.round=1;room.phase='supply';room.deadline=Date.now()+5000;supplySolar(room);room.log.unshift(`${room.players.length}명으로 게임 시작`)}
function supplySolar(room:Room){room.supplyRecipients={};if(room.round>4)return;for(const p of shuffle(room.players).slice(0,4)){p.inventory.push({id:uid(),type:'태양E',path:'태양'});room.supplyRecipients[p.id]=(room.supplyRecipients[p.id]||0)+1}room.log.unshift('태양E 조각 4개 무작위 공급')}
export function advance(room:Room,negotiationSeconds=60){
 if(room.phase==='lobby')return start(room);
 if(room.phase==='supply'){room.phase='negotiation';room.deadline=Date.now()+negotiationSeconds*1000;room.log.unshift(`${room.round}R 협상`);return}
 if(room.phase==='negotiation'){room.phase='secret';room.deadline=Date.now()+20000;room.players.forEach(p=>{p.converted=false;p.transferred=false;p.inbox=[];delete p.pendingTransfer});room.log.unshift(`${room.round}R 비밀 행동`);return}
 if(room.phase==='secret'){resolveTransfers(room);room.phase='result';room.deadline=Date.now()+8000;room.log.unshift(`${room.round}R 결과 확인`);return}
 if(room.phase==='result'){if(room.round>=6){room.phase='guess';room.deadline=null;room.log.unshift('최종 역할 추리')}else{room.round++;if(room.round<=4){room.phase='supply';room.deadline=Date.now()+5000;supplySolar(room)}else{room.phase='negotiation';room.deadline=Date.now()+negotiationSeconds*1000;room.supplyRecipients={};room.log.unshift(`${room.round}R FINAL PHASE 협상`)}}return}
...
export function scores(room:Room){const commonScore=room.paths.length,guessedBy=new Map<string,{name:string;role:string;correct:boolean}[]>();for(const p of room.players){for(const g of p.guesses||[]){const target=room.players.find(x=>x.id===g.toId);if(target){const list=guessedBy.get(g.toId)||[];list.push({name:p.name,role:g.role,correct:target.role===g.role});guessedBy.set(g.toId,list)}}}return room.players.map(p=>{const judged=(p.guesses||[]).map(g=>{const target=room.players.find(x=>x.id===g.toId);return!!target&&target.role===g.role}),guessCorrectCount=judged.filter(Boolean).length,guessWrongCount=judged.length-guessCorrectCount,guessPoints=guessCorrectCount-guessWrongCount,personal=personalSuccess(p),named=(guessedBy.get(p.id)||[]).some(x=>x.correct);return{id:p.id,name:p.name,role:p.role,commonScore,personal,guessCorrectCount,guessWrongCount,guessPoints,revealed:named,guessedBy:guessedBy.get(p.id)||[],score:commonScore+(personal?3:0)+guessPoints-(named?1:0)}})}
