...
export type Player = { id:string; token:string; name:string; role:string; inventory:Energy[]; converted:boolean; transferred:boolean; inbox:string[]; stats:{made:number;converted:number;sent:number;received:number;car:number}; guess?:string };
...
const conversion:Record<string,{from:EnergyType;to:EnergyType;needs?:string;tag?:string}> = {
...
export function join(room:Room,name:string){ if(room.phase!=='lobby')throw Error('이미 시작된 조입니다.'); if(room.players.length>=6)throw Error('이 조는 6명이 모두 참여했습니다.'); if(room.players.some(p=>p.name===name))throw Error('같은 이름이 이미 있습니다.'); const p:Player={id:uid(),token:uid(),name,role:'',inventory:[],converted:false,transferred:false,inbox:[],stats:{made:0,converted:0,sent:0,received:0,car:0}}; room.players.push(p); return p; }
export function start(room:Room){ if(room.players.length<2)throw Error('최소 2명이 참여해야 시작할 수 있습니다.'); const optional=shuffle(ROLE_SETS[room.set].filter(r=>r!=='태양'&&r!=='자동차')); const roles=shuffle(['태양','자동차',...optional.slice(0,room.players.length-2)]); room.players.forEach((p,i)=>p.role=roles[i]); room.round=1; room.phase='negotiation'; room.deadline=Date.now()+120000; room.log.unshift(`${room.players.length}명으로 게임 시작 · 1라운드 협상`); }
...
export function convertEnergy(room:Room,p:Player,energyId:string){ if(room.phase!=='secret'||p.converted)throw Error('지금은 전환할 수 없습니다.'); const rule=conversion[p.role]; if(!rule)throw Error('이 역할은 직접 전환하지 않습니다.'); const i=p.inventory.findIndex(e=>e.id===energyId&&e.type===rule.from&&(!rule.needs||e.path.includes(rule.needs))); if(i<0)throw Error('전환 가능한 에너지가 없습니다.'); const e=p.inventory[i]; e.type=rule.to; if(rule.tag)e.path+=`→${rule.tag}`; p.converted=true;p.stats.converted++;room.log.unshift(`${p.name}이(가) 에너지를 전환함`); }
function pathKey(path:string){if(path.includes('태양광'))return '태양광 경로';if(path.includes('바람'))return '풍력 경로';if(path.includes('물'))return '수력 경로';if(path.includes('화력'))return '화력 경로';return '화석연료 경로'}
export function transfer(room:Room,p:Player,toId:string,energyId:string){ if(room.phase!=='secret'||p.transferred)throw Error('지금은 전달할 수 없습니다.'); if(toId===p.id)throw Error('자기 자신에게는 전달할 수 없습니다.'); const to=room.players.find(x=>x.id===toId); const i=p.inventory.findIndex(e=>e.id===energyId); if(!to||i<0)throw Error('전달 대상을 확인해 주세요.'); const [e]=p.inventory.splice(i,1);p.transferred=true;p.stats.sent++;to.stats.received++;
  if(to.role==='자동차'&&(e.type==='전기E'||(e.type==='화학E'&&e.path.includes('화석연료')))){const key=pathKey(e.path);if(!room.paths.includes(key))room.paths.push(key);to.stats.car++;to.inbox.unshift(`${e.type} 수신 → 운동E 자동 전환 · ${key} 완성!`);room.log.unshift(`자동차 경로 완성: ${key}`);}else{to.inventory.push(e);to.inbox.unshift(`${e.type} 1개를 익명으로 받음`);room.log.unshift(`익명으로 ${e.type} 전달됨`);}}
export function personalSuccess(p:Player,room:Room){if(p.role==='태양')return p.stats.sent>=2;if(p.role==='자동차')return room.paths.length>=2;if(['식물','대기·바람','물순환'].includes(p.role))return p.stats.converted>=2;return p.stats.car>=1;}
