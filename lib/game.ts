...
function supplySolar(room:Room){if(room.round>6)return;for(const p of shuffle(room.players).slice(0,4)){p.inventory.push({id:uid(),type:'태양E',path:'태양'});p.inbox.unshift(`${room.round}라운드 태양E 조각 1개 공급`)}room.log.unshift('태양E 조각 4개 무작위 공급')}
export function advance(room:Room,negotiationSeconds=120){
 if(room.phase==='lobby')return start(room,negotiationSeconds);
 if(room.phase==='negotiation'){room.phase='secret';room.deadline=Date.now()+40000;room.players.forEach(p=>{p.converted=false;p.transferred=false;delete p.pendingTransfer});supplySolar(room);room.log.unshift(`${room.round}R 비밀 행동`);return}
