...
export function start(room:Room){ if(room.players.length!==6)throw Error('6명이 모두 참여해야 시작할 수 있습니다.'); const roles=shuffle(ROLE_SETS[room.set]); room.players.forEach((p,i)=>p.role=roles[i]); room.round=1; room.phase='negotiation'; room.deadline=Date.now()+120000; room.log.unshift('게임 시작 · 1라운드 협상'); }
