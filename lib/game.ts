...
export type Player = { id:string; token:string; name:string; role:string; inventory:Energy[]; converted:boolean; transferred:boolean; pendingConvert?:string; pendingTransfer?:{toId:string;energyId:string}; inbox:string[]; stats:{made:number;converted:number;sent:number;received:number;car:number;paths:string[]}; guess?:string };
...
export function advance(room:Room){
  ...
  if(room.phase==='negotiation'){ room.phase='secret'; room.deadline=Date.now()+30000; room.players.forEach(p=>{p.converted=false;p.transferred=false;delete p.pendingConvert;delete p.pendingTransfer}); if(room.round<=4){const sun=room.players.find(p=>p.role==='태양'); if(sun){sun.inventory.push({id:uid(),type:'태양E',path:'태양'});sun.stats.made++;sun.inbox.unshift(`${room.round}라운드 태양E 1개 생성`)}} room.log.unshift(`${room.round}R 비밀 행동`); return; }
  if(room.phase==='secret'){resolveActions(room);room.phase='result';room.deadline=Date.now()+30000;room.log.unshift(`${room.round}R 결과 확인`);return;}
...
function conversionFor(p:Player,e:Energy){return conversion[p.role]?.find(r=>e.type===r.from&&(!r.needs||e.path.includes(r.needs)))}
export function convertEnergy(room:Room,p:Player,energyId:string){if(room.phase!=='secret'||p.converted)throw Error('지금은 전환을 제출할 수 없습니다.');const e=p.inventory.find(x=>x.id===energyId);if(!e||!conversionFor(p,e))throw Error('전환 가능한 에너지가 없습니다.');p.pendingConvert=energyId;p.converted=true;}
...
export function transfer(room:Room,p:Player,toId:string,energyId:string){if(room.phase!=='secret'||p.transferred)throw Error('지금은 전달을 제출할 수 없습니다.');if(toId===p.id)throw Error('자기 자신에게는 전달할 수 없습니다.');if(!room.players.some(x=>x.id===toId)||!p.inventory.some(e=>e.id===energyId))throw Error('전달 대상을 확인해 주세요.');p.pendingTransfer={toId,energyId};p.transferred=true;}
function applyTransfer(room:Room,p:Player,toId:string,energyId:string){const to=room.players.find(x=>x.id===toId);const i=p.inventory.findIndex(e=>e.id===energyId);if(!to||i<0)return;const [e]=p.inventory.splice(i,1);p.stats.sent++;to.stats.received++;
...
function resolveActions(room:Room){for(const p of room.players){if(!p.pendingConvert)continue;const e=p.inventory.find(x=>x.id===p.pendingConvert);const rule=e&&conversionFor(p,e);if(e&&rule){e.type=rule.to;if(rule.tag)e.path+=`→${rule.tag}`;p.stats.converted++;room.log.unshift(`${p.name}이(가) 에너지를 전환함`);}}for(const p of room.players){if(p.pendingTransfer)applyTransfer(room,p,p.pendingTransfer.toId,p.pendingTransfer.energyId);}}
